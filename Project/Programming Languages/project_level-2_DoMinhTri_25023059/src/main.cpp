...
int main(int argc, char *argv[])
{
    ...
    SDLInteraction *sdlInter = nullptr;
    ...
    else
    {
        ...
        auto *sdlRend = dynamic_cast<SDLRenderer *>(iRenderer);
        auto *sdlAudio = new Audio();
        sdlAudio->init();
        sdlRend->setAudio(sdlAudio);

        sdlInter = new SDLInteraction();
        sdlInter->setRenderer(sdlRend);
        iInteraction = sdlInter;
    ...
    while (keepRunning)
    {
        try
        {
            if (config.gui_flag && sdlInter)
            {
                sdlInter->handleMainMenu();
            }
